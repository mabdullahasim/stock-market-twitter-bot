...
def get_market_gainers():
    ...
    gainers = []
    response = requests.get(URL)         #sends http request to the URL gets full html content of page
    ...
    for row in top_rows:                                #iterates through each row in the table
        ...
        percent_change = cells[1].text.strip()      #inside the second td it finds the percent change
        pre_market_volume = cells[4].text.strip()   #inside the fourth td it finds the pre-market volume
        percent_change = float(percent_change.replace("%","")) # convert the string number into a float
...
def get_earnings_calendar():
    ...
    payload = { #payload to be sent with the request
        ...
        "columns": [ #columns tells the url which fields you want
            "name",
